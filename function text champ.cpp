...
pair<string, int> generateMarketNews() {
    ...
    if (news.empty()) return {"No news available.", -1};

    int index = rand() % news.size(); // สุ่มบรรทัด
    return {news[index], index + 1};  // index + 1 เพื่อให้เริ่มที่บรรทัดที่ 1 ไม่ใช่ 0
}
...
int main() {
    srand(time(0));
    pair<string, int> news = generateMarketNews();
    displayTextArt();
